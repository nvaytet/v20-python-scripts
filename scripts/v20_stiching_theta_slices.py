...
ws_raw = LoadEventNexus(FileName=theFile, LoadLogs=False)
...
# xmax = 5.0e4
    ...
    for i in range(nbins):
        x_tof[i] = 0.5*(x_edges[i] + x_edges[i+1])
    ax[k].plot(x_tof,normalized_wav.extractY()[0], label=r"$\theta$"+" = {}".format(0.5*(th1+th2)), color='k')
    # ax3.set_xlim([xmin, xmax])
    ax[k].set_xlabel(r"Wavelength ($\AA$)")
    ax[k].set_ylabel("Arbitrary units")
    ax[k].legend()

fig.savefig('stitched_theta_slices.pdf',bbox_inches='tight')
