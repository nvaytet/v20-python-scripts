...
# theFile = "V20_ESSIntegration_2018-12-14T09:25:00"
theFile = "v20-2018-12-14T11:23:38+0100"
# ws = LoadEventNexus(FileName=dirpath+theFile+"/"+theFile+"_stripped.nxs", LoadLogs=True)





ws_mon = LoadNexusMonitors(dirpath+theFile+"/"+theFile+"_stripped.nxs", LoadOnly='Events')
...
# Rebin the data
temp = Rebin(ws_mon, "{},{},{}".format(tof_min,binsize,tof_max))


summed = CreateWorkspace(DataX=temp.extractX()[1], DataY=temp.extractY()[1], NSpec=1, UnitX="TOF")
...
#     # And plot
#     ax1.plot(x_mon,y_mon[j,:], label="Monitor {} ({} events)".format(j+1,nevents_mon[j]))
#
# ax1.set_xlim([tof_min,tof_max])
# ax1.set_xlabel("Arrival time at detector (microseconds)")
# ax1.set_ylabel("Counts (integrated over x)")
# ax1.legend()
#
# fig.savefig("monitor_stitched.pdf", bbox_inches="tight")
# exit()

# Empty Vanadium can
vanFile = "v20-2018-12-14T16:12:26+0100"
van_mon = LoadNexusMonitors(dirpath+vanFile+"/"+vanFile+"_stripped.nxs", LoadOnly='Events')


# Rebin the data
temp2 = Rebin(van_mon, "{},{},{}".format(tof_min,binsize,tof_max))
van = CreateWorkspace(DataX=temp2.extractX()[1], DataY=temp2.extractY()[1], NSpec=1, UnitX="TOF")
...
frame_parameters1 = get_wfm_windows(data=summed_data, output_format=output_format, plot=True, win_threshold=0.30)
...
frame_parameters2 = get_wfm_windows(data=summed_data_van, output_format=output_format, plot=True, win_threshold=0.30)
